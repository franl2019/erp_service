...
export class OutboundMxEntity {
    ...
    public async find(outboundid: number): Promise<IFindOutboundMx[]> {
        ...
        const sql = `SELECT
                    ...
                    outbound_mx.otherUnit,
                    outbound_mx.otherUnitConversionRate,
                    
                    outbound_mx.kz_productCode,
                    outbound_mx.kz_productName,
                    outbound_mx.kz_spec,
                    outbound_mx.kz_materials,
                    outbound_mx.kz_remark,
                    outbound_mx.kz_spec_d,
                    outbound_mx.kz_materials_d,
    ...
    public async find_entity(outboundid: number): Promise<IOutboundMx[]> {
        ...
        if ((res as IOutboundMx[]).length > 0) {
            return res as IOutboundMx[];
        } else {
            return Promise.reject(new Error("查找出仓单的明细错误"));
        }
    }
